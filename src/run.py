...
# JUNK code #
                ...
                # # -- Constraints file ---
                # if args.cons != "":
                #     # check that the constraints_path jsonl exists
                #     if not os.path.exists(args.cons):
                #         print(f"*** ERROR ***: Constraints file {args.cons} does not exist.")
                #         return
                #     # open and read the .jsonl file
                #     with open(args.cons, 'r') as file:
                #         data = [json.loads(line) for line in file]  

# def replace_constraints(data, spec: tl.Specification, M:int, N:int, K:int):
#     # find the entry in data with the args M N and K
#     constraints = None
#     for entry in data:
#         if entry["input_arguments"]["M"] == M and entry["input_arguments"]["N"] == N and entry["input_arguments"]["K"] == K:
#             constraints = entry["constraints"]
#             break

#     if constraints is None:
#         print(f"*** ERROR ***: Constraints for {M}x{N}x{K} not found.")
#         return

#     # update the spec with the constraints
#     tmp = spec.constraints.targets
#     # update this to go over the list of all targets instead of relying on the order
#     if tmp[5].target == 'DRAM':
#         spec.constraints.targets[5].factors = Factors.factory(constraints["DRAM"]) #DRAM temporal constraints
#     if tmp[6].target == 'SMEM':
#         spec.constraints.targets[6].factors = Factors.factory(constraints["SMEM"]) #DRAM temporal constraints
#     if tmp[9].target == 'CiMStorage':
#         spec.constraints.targets[9].factors = Factors.factory(constraints["CiMStorage"])
#     if tmp[10].target == 'CiMArray':
#         spec.constraints.targets[10].factors = Factors.factory(constraints["CiMArray"])
#     if tmp[11].target == 'CiMBank':
#         spec.constraints.targets[11].factors = Factors.factory(constraints["CiMBank"])
